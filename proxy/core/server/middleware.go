...
func Middleware(writer http.ResponseWriter, request *http.Request) {
	...
	if !strings.Contains(request.Header.Get("Cookie"), "_zux_v="+encryptedIP) {
		...
		case 1:
			writer.Header().Set("Set-Cookie", "_1_zux_v="+encryptedIP+"; SameSite=Lax; path=/; Secure")
		...
		case 3:
			...
			if !captchaExists {
				...
				if err := png.Encode(&captchaBuf, captchaImg); err != nil {
					SendResponse("zuxent Error: Failed to encode captcha: "+err.Error(), buffer, writer)
					return
				}
				if err := png.Encode(&maskBuf, maskImg); err != nil {
					SendResponse("zuxent Error: Failed to encode captchaMask: "+err.Error(), buffer, writer)
					return
				}
	...
	case "/zux/verified":
		...
		return
	case "/zux/" + proxy.AdminSecret + "/api/v1":
	...
	if strings.HasPrefix(request.URL.Path, "/zux/api/v2") {
